  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('Error submitting form: ' + (errorData.errors?.join(', ') || 'Status: ' + response.status));
      }
    } catch (error) {
      console.error('Network error:', error);
      alert('Network error: Make sure your backend server is running on localhost:5000');
    }
  }
